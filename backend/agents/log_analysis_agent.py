...
Be specific and actionable in your analysis."""
    ...
    def __init__(self, llm):
        ...
        self.prompt = PromptTemplate(
            template=self.ANALYZE_PROMPT, input_variables=["role", "logs", "pattern"]
        )
    ...
    def execute(self, role: str, logs: str, pattern: str = "general") -> str:
        ...
        result = chain.invoke({"role": role, "logs": logs, "pattern": pattern})
    ...
    def extract_metrics(self, logs: str) -> Dict[str, Any]:
        ...
        for line in logs.splitlines():
            ...
            if any(
                ...
                for keyword in ["failed", "error", "denied", "blocked", "rejected"]
            ):
                events["failed"] += 1
                error_lines.append(line)
            elif any(
                keyword in line_lower for keyword in ["success", "accepted", "allowed"]
            ):
                events["success"] += 1
            else:
                events["other"] += 1
