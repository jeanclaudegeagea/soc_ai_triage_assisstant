...
def _parse_corr_verdict(corr: Dict[str, Any]) -> str:
    ...
    if structured_verdict in ["TRUE_POSITIVE", "FALSE_POSITIVE", "REVIEW_NEEDED"]:
        return structured_verdict.replace("_", " ")
...
def build_detailed_soc_pdf(
    ...
    mitre_summary: Dict[str, Any] = full_analysis.get("mitre_summary") or {}
    ...
    meta_data = [
        ...
        ["MITRE Techniques", str(mitre_summary.get("unique_technique_count", 0))],
    ...
    elements.append(Paragraph("3. MITRE ATT&CK Mapping Summary", h_style))
    technique_rows = (mitre_summary.get("techniques") or [])[:12]
    if technique_rows:
        mitre_table_data = [["Technique ID", "Technique Name", "Tactic", "Count"]]
        for row in technique_rows:
            mitre_table_data.append(
                [
                    str(row.get("technique_id", "N/A")),
                    str(row.get("technique_name", "Unknown")),
                    str(row.get("tactic", "Unknown")),
                    str(row.get("count", 0)),
                ]
            )
        mitre_table = Table(mitre_table_data, colWidths=[1.0 * inch, 2.8 * inch, 1.2 * inch, 0.8 * inch])
        ...
        elements.append(Paragraph("No MITRE ATT&CK mappings were generated.", body_style))
    elements.append(PageBreak())

    # Attack findings
    elements.append(Paragraph("4. Detailed Attack Findings", h_style))
    if not attack_details:
        ...
        for idx, attack in enumerate(attack_details, start=1):
            ...
            if mitre_map:
                elements.append(Paragraph("<b>MITRE ATT&CK Mapping</b>", body_style))
                for m in mitre_map:
                    line = (
                        f"- {escape(str(m.get('technique_id', 'N/A')))} "
                        f"{escape(str(m.get('technique_name', 'Unknown')))} "
                        f"({escape(str(m.get('tactic', 'Unknown')))}), "
                        f"confidence {escape(str(m.get('confidence', 0)))}%"
                    )
    ...
    # Correlation
    elements.append(Paragraph("5. Correlation and False Positive Review", h_style))
    if not correlation_results:
        ...
        for idx, corr in enumerate(correlation_results, start=1):
            ...
            elements.append(
                Paragraph(
                    f"<b>Confidence:</b> {escape(str(corr.get('confidence', 0)))}%",
                    body_style,
                )
            )
            elements.append(
                Paragraph(
                    f"<b>Attack Campaign:</b> {escape(str(corr.get('attack_campaign', 'N/A')))}",
                    body_style,
                )
            )
            ...
            elements.append(Paragraph(_safe_lines(str(corr.get("reasoning", "N/A"))), body_style))
            corr_recs = corr.get("recommendations", [])
            if isinstance(corr_recs, list) and corr_recs:
                elements.append(Paragraph("<b>Recommended Actions</b>", body_style))
                for rec in corr_recs:
                    elements.append(Paragraph(f"- {_format_inline_markdown(str(rec))}", body_style))
    ...
    elements.append(Paragraph("6. Recommended Response Plan", h_style))
    if recommendations:
    ...
    elements.append(Paragraph("7. CVE References", h_style))
