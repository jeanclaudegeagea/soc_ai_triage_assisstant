...
def generate_pdf(report, role):
    ...
    severity_data = [[Paragraph(f"<b>SEVERITY: {report['severity'].upper()}</b>", 
                                ParagraphStyle('SevText', parent=body_style, 
    ...
    for idx, rec in enumerate(report["recommendations"], 1):
        ...
        rec_data.append([
            ...
            Paragraph(f"<b>{priority}</b>", 
                     ParagraphStyle('Priority', parent=body_style, 
    ...
    footer_data = [[
        Paragraph("<i>This report is confidential and intended for authorized personnel only.</i>", 
                 ParagraphStyle('FooterText', parent=body_style, fontSize=8, 
                              textColor=colors.grey, alignment=TA_CENTER))
    ]]
